...
export const SwaggerModel = types
    .compose(
        ToolModel,
        types.model({
            autoInitUI: types.optional(
                types.boolean,
                config.cr.common.autoInitUI
            ),
            recaptchaSiteKey: types.optional(
                types.string,
                config.cr.common.recaptchaSiteKey
            ),
            loginWithOtp: types.optional(
                types.boolean,
                config.cr.common.loginWithOtp
            ),
            otpCode: types.optional(types.string, ""),
            adminIamUserId: types.optional(
                types.string,
                config.cr.admin.iamUserId
            ),
            ...
            performerIamUserId: types.optional(
                types.string,
                config.cr.performer.iamUserId
            ),
            performerEmail: types.optional(
                types.string,
                config.cr.performer.email
            ),
            performerId: types.optional(
                types.string,
                config.cr.performer.accountId
            ),
            userIamUserId: types.optional(
                types.string,
                config.cr.user.iamUserId
            ),
            ...
            appInstanceCode: types.optional(
                types.string,
                "SEEDING_APP_INSTANCE_1_CODE"
            ),
        })
    )
    .named("SwaggerModel")
